...
export default function App() {
  const [todos, setTodos] = useState([
    { id: 1, text: 'Comprar despensa', completed: false },
    ...
    <div className="min-h-screen bg-gradient-to-br from-violet-100 via-purple-50 to-indigo-100 flex items-start justify-center pt-16 px-4">
      <div className="w-full max-w-md">
        ...
        <div className="flex gap-2 mb-6">
          ...
          <button
            onClick={addTodo}
            className="bg-purple-600 hover:bg-purple-700 active:bg-purple-800 text-white font-semibold px-5 py-3 rounded-xl shadow-sm transition-colors cursor-pointer"
          >
            Agregar
          </button>
        </div>
        ...
        <div className="bg-white rounded-2xl shadow-lg overflow-hidden">
          ...
          <ul className="divide-y divide-gray-100">
            {filtered.length === 0 && (
              <li className="py-10 text-center text-gray-400 text-sm">
                {filter === 'Completadas' ? 'Sin tareas completadas.' : '¡Sin tareas pendientes!'}
              </li>
            )}
            {filtered.map(todo => (
              <li
              ...
              >
                ...
                <button
                  ...
                  className="opacity-0 group-hover:opacity-100 text-gray-300 hover:text-red-500 transition-all text-xl leading-none cursor-pointer"
          ...
          <div className="flex items-center justify-between px-5 py-3 bg-gray-50 border-t border-gray-100 text-xs text-gray-400">
            <span>{activeCount} tarea{activeCount !== 1 ? 's' : ''} pendiente{activeCount !== 1 ? 's' : ''}</span>
